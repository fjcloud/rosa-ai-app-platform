...
Audits change files, so stay in *build* mode. Press kbd:[Tab] until the TUI shows build.

== Audit 1: Resource Governance

[source,text,role=execute]
----
Check resource requests and limits on deploy/base/deployment.yaml. Fix if needed.
----

If it patches the file, tell it to ship the change.

== Audit 2: Observability

[source,text,role=execute]
----
Check liveness and readiness probes on the Deployment. Fix if needed.
----
...
[source,text,role=execute]
----
Audit pod security and Route TLS in deploy/base/. Fix if needed.
----
...
| Fortune Cookie app generated live by OpenCode + Qwen3.8
...
➡️ Move on to *Act 3: The bill* — GPU watts and token quota from this afternoon.
