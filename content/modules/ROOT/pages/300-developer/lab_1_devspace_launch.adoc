...
The Dev Space workspace pod runs as a service account with limited permissions.
Log in with your personal credentials so the Ansible playbooks can create namespaces and deploy resources:

[source,sh,role=execute]
----
oc login {rosa_openshift_api_url} \
  -u {rosa_openshift_admin_user} \
  -p {rosa_openshift_admin_password}
----
...
This should return your username, not a service account. All `oc` commands and the Ansible playbooks will now use your credentials.

== Verify OpenCode is Running

. Open a terminal in the workspace (*Terminal → New Terminal* or press kbd:[Ctrl+`]).

. Check that OpenCode, gitpop, and Ansible were installed by the `postStart` hook:
+
[source,sh,role=execute]
----
...
----
+
The OpenCode TUI (Terminal UI) launches. It reads `AGENTS.md` and connects to the Qwen3.6 LLM automatically.
